...
const PaymentSelection = () => {
	...
	const buttonDisable = useSelector(
		(state: State) => state.payment.buttonDisable
	);
	const { placeOrder, deleteCart, updateCart } = bindActionCreators(
		authActionCreators,
		dispatch
	);
	const { createRazorpayOrder } = bindActionCreators(
		paymentActionCreators,
		dispatch
	);
	...
	const orderPlacementHandler = () => {
		...
		if (value === 'RZP') {
			createRazorpayOrder(orderObject, user, setopen);
			return;
		}
